...
The grid creates visual consistency between layouts while allowing flexibility across a wide variety of designs. Sendo Design’s responsive UI is based on a 12-column grid layout.
...
#### Spacing
Sendo Design margins and columns follow an 4px square baseline grid. The spacing property is an integer between 0 and 12 inclusive. By default, the spacing between two grid items follows a linear function: output(spacing) = spacing * 4px, e.g. spacing={2} creates a 8px wide gap.

```tsx
import { Grid, Paper, Radio, RadioGroup, Typography, TypoVariants, TypoWeights, useRadio } from '@sendo/ui';
...
```tsx
import { Grid, Paper } from '@sendo/ui';
